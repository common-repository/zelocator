...
class HC3_Ui implements HC3_Ui_
{
	...
	public function makePercent( $number, $digits = 2, $arrowChange = FALSE )
	{
		...
		if( $number > 0 ){
			$return->styleColor( 'olive' );
		}
		elseif( $number < 0 ) {
			$return->styleColor( 'maroon' );
		}
	...
	{
		if( is_array($label) ){
			$return = new HC3_Ui_Element_Input_CheckboxMagic( $this, $name, $label, $value, $checked );
		}
		else {
	...
	{
		if( is_array($label) ){
			$return = new HC3_Ui_Element_Input_RadioMagic( $this, $name, $label, $value, $checked );
		}
		else {
...
// $noLimit = TRUE;
	...
	public function makeInputTime( $name, $label = NULL, $value = array(), $noLimit = FALSE )
	{
		$timeFormatOptions = array();
