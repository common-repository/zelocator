...
abstract class HC3_Abstract_Plugin
{
	...
	public function _init()
	{
		...
		$profiler->markStart( 'total' );

		$dic = new HC3_Dic;
		$dic->bind( $profiler );

		$hooks = new HC3_Hooks( $dic, $this->slug, $this->prefix, $profiler );
		$dic->bind( $hooks );

		$crudFactory = new HC3_CrudFactory( $this->prefix );
		$session = new HC3_Session( $this->slug );
		$settings = new HC3_Settings( $this->prefix );
		$transients = new HC3_Transients( $this->prefix );
